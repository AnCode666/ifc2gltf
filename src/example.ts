...
/* AnCode 1º: import gltfExporter and fragmentsGroup */
...
/* AnCode 2º: object returned by ifcLoader must be accesible from other functions, so must be declared here */
var model: FragmentsGroup;
...
// THIS IS DIFFERENT FROM THE OFFICIAL EXAMPLE
...
/* AnCode 3º: The function to download the GLB file */
function downloadGLBFile(file: BlobPart) {

  const blob = new Blob([file]);

  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = 'IFC_model.glb';
  document.body.appendChild(link);
  link.click();
  link.remove();
}
...
/*
AnCode 4º: Function to convert fargments to glTF format. Uses a three.js add-on called GLTFExporter
...
function exportGLB(){
  ...
  if (model){
    ...
    // Define options of the exporter
    const options = {
      //true to export glb (binario), false to export gltf (jSON)
      binary: true, 
      // this must be set to true to be more compatible with gl extensions,
      includeCustomExtensions: true 
    };

    // Parse the model and generate the gltf/glb output
    exporter.parse(
      model,
      
      // called when the gltf/glb has been generated
      function (glb: any) {
          console.log('Generated GLB: ', glb);
    ...
    alert ('You must load an IFC model first');
  }
  ...
  <bim-panel active label="IFC Loader Tutorial" class="options-menu">
    <bim-panel-section collapsed label="Controls">
      <bim-panel-section style="padding-top: 12px;">
        ...
        <bim-button label="Export GLB file"
          @click="${() => {
            exportGLB();
          }}">
        </bim-button>
